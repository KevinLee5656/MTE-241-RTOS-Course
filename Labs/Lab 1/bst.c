 ...
 ****************************************************************************/
 ...
 /*
 * Names:
 *   1.  uWaterloo User ID:  es7lee@uwaterloo.ca
 *   2.  uWaterloo User ID:  k27hong@uwaterloo.ca
 */
...
*               to main.c or bst.c.
*****************************************************************
*  1. bsn_init: Removed the unnecessary whitespaces between the parameters and brackets to establish consistency in spacing.
*  2. bsn_init: Indented the body lines to make the code easier to read.
*  3. bsn_init: Moved the first of two braces up one line to condense the code.
*  4. bst_delete: Modified "case" comment, such that case 2 precedes case 3, to establish numerical order.
*  5. bst_erase: Removed braces from one-line if/else-if/else statements to condense the code.
****************************************************************/
...
*****************************************************************
*  1. bst_delete: Changed p_swapNode->left to p_swapNode-> right. Tracebacked error 65 from Command Window.
*  2. bst_erase: Changed +=1 to -=1 since tree is decrementing. Used step buttons after running "Before First Group" print statement.
*  3. bst_delete: Changed parentNode->right to ->left and parentNode->left to ->right in order to correspond with if statement. 
                  Added p_myTree to Watch Window and observed the changes in left and right nodes.
									More detail: In main, we call bst_erase to delete nodes with certain values (listed in array). 
															 bst_erase starts from the root and traverses through the nodes until it finds the passed in value.
															 There are two cases: 1) the value is in the root, 2) the value is not in the root
															 The expected outcome of case 3 in bst_delete is that the to-be-deleted node has one child. 
															 And we need to replace the deleted node with the single child. 
															 However, it was observed in the watch window that the current node that was on the left of the parent was 
															 being replaced by the right child of the current node, which was NULL. 
															 The left of the parent should have been replaced by the right child of the current node.
*  4. bst_delete: Replaced & with operator && in the else if statement. Found it while looking for logic bug #1. (No effect)
****************************************************************/
...
void bst_delete(bsn_t *p_currNode, bsn_t *p_parentNode) {
	...
	// balancing was not a parameter, so we will always take max of left branch to replace it with
	else if((p_currNode->right != NULL) && (p_currNode->left != NULL)) { // change 4 (& -> &&)
		...
		while(p_swapNode->right != NULL) {
			...
			p_swapNode = p_swapNode->right; // change 1 (left -> right)
		}
	...
	// case 3: one child, replace with child, delete child
		...
		if(p_currNode->left != NULL) { //child is to the left
			...
			// need to know whether we are to the left or right of the parent
			if(p_parentNode->left == p_currNode) {
				p_parentNode->left = p_currNode->right; // change 3 (right to left, left to right)
			}
			else {
...
bool bst_erase(bst_t *tree, S32 val) {
	...
	while(p_currNode != NULL) {
		if(p_currNode->val == val) {
			...
			tree->size -= 1;	//decrement size of the tree; change 2 (+= to -=)
			return __TRUE;
		}
		else if(val < p_currNode->val) { 
			//move left in tree
